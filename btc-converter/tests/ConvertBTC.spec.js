...
describe('ConvertBTC', () => {
  ...
  it('should use currency USD and 1 as amount default', (done) => {
    ...
    setTimeout(() => {
      expect(consoleStub).to.have.been.calledWith(`${chalk.red(1)} BTC to ${chalk.cyan('USD')} = ${chalk.yellow(16082.23)}`);
      done();
    }, 300);
  });
  ...
  it('should use currency USD and 10 as amount', (done) => {
    ...
    setTimeout(() => {
      expect(consoleStub).to.have.been.calledWith(`${chalk.red(10)} BTC to ${chalk.cyan('USD')} = ${chalk.yellow(16082.23)}`);
      done();
    }, 300);
  });
  ...
  it('should use currency BRL and 1 as amount default', (done) => {
    ...
    setTimeout(() => {
      expect(consoleStub).to.have.been.calledWith(`${chalk.red(1)} BTC to ${chalk.cyan('BRL')} = ${chalk.yellow(16082.23)}`);
      done();
    }, 300);
  });
